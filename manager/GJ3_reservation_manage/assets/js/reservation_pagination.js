...
function pagenation(selector, data) {
    ...
    a1.setAttribute("href", "/GJ3_reservation_manage/reservation_manage.html?page=" + data.prevGroupLastPage + "&query=" + query);

    if(!(order == null)){
        a1.setAttribute("href", "/GJ3_reservation_manage/reservation_manage.html?page=" + data.prevGroupLastPage + "&query=" + query +  "&order=" + order)
    }
    ...
    for (let i = data.groupStart; i <= data.groupEnd; i++) {
        ...
        a2.setAttribute("href", "/GJ3_reservation_manage/reservation_manage.html?page=" + i + "&query=" + query);

        if (!(order == null)) {
            a2.setAttribute("href", "/GJ3_reservation_manage/reservation_manage.html?page=" + i + "&query=" + query + "&order=" + order);
        }
    ...
    a3.setAttribute("href", "GJ3_reservation_manage/reservation_manage.html?page=" + data.nextGroupFirstPage + "&query=" + query);

    if (!(order == null)) {
        a3.setAttribute("href","GJ3_reservation_manage/reservation_manage.html?page=" + data.nextGroupFirstPage + "&query=" + query + "&order=" + order);
    }
