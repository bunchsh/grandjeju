...
app.use((req, res, next) => {
    ...
    logger.debug('[client의 UserAgent]' + ip + ' / ' + req.useragent.os + ' / ' + req.useragent.browser + ' (' + req.useragent.version + ') / ' + req.useragent.platform)
    ...
    res.on('finish', () => {
        ...
        logger.debug('※※※※※ 클라이언트의 접속이 종료되었습니다. ※※※※※    [실행하는데 걸린시간 runtime] :' + time + 'ms');
        logger.debug('----------------------------------------');
    });
...
app.use(expressSession({
    ...
    store: new ExpressMysqlSession(config.GJ_database2)
}));
