...
module.exports = (app) => {
    ...
    /** 특정 항목에 대한 상세 조회 --> Read(SELECT) (추가) */
    router.get("/memberss/:member_id", async (req, res, next) => {
        ...
        try {
            ...
            // 데이터 조회
            const sql = "SELECT member_id, user_id, user_pw, user_name, user_phone, date_format(reg_date,'%Y/%m/%d %H:%i') as reg_date, is_admin FROM members WHERE member_id=?";
            const [result] = await dbcon.query(sql, [member_id]);
